...
def change1(line):
 reason = 'marked'
 newline = re.sub(r'</ls>([;.,]) *$',r'</lsx>\1',line)
 ...
 for iline,line in enumerate(lines):
  ...
  for f in change_fcns:
   ...
   if newline != oldline:
    ...
    change = Change(metaline,page,iline,oldline,newline1,reason,iline1,line1)
...
def change_out(change,ichange):
 ...
 # prior line is dummy
 lnum = change.iline + 1
 outarr.append(';x %s old %s' % (lnum,change.old))
 ...
 outarr.append('%s new %s' % (lnum1,change.new))
 outarr.append(';')
 return outarr
