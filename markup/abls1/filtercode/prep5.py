...
class Abbrv(object):
 def __init__(self,line):
  ...
  self.line = line
  parts = line.split(':')
  (self.abbr,self.code,self.idxstr,self.tooltip,countstr,numnumstr,maxstr) = parts[0:7]
...
regex_u2_raw = r'<ls>([^<]*?) ([0-9]+[.] [0-9]+[.]?)</ls>([,;]) ([0-9]+[.] [0-9]+[.]?)'
regex_u2 = re.compile(regex_u2_raw)
def update_2(line):
 newline = line
 while True:
  m = re.search(regex_u2,newline)
  if not m:
   break
  newline = re.sub(regex_u2,r'<ls>\1 \2</ls>\3 <ls n="\1">\4</ls>',newline)
 return newline

regex_u2a_raw = r'<ls n="([^"]*?)">([0-9]+[.] [0-9]+[.]?)</ls>([,;]) ([0-9]+[.] [0-9]+[.]?)'
regex_u2a = re.compile(regex_u2a_raw)
def update_2a(line):
 newline = line
 while True:
  m = re.search(regex_u2a,newline)
  if not m:
   break
  newline = re.sub(regex_u2a,r'<ls n="\1">\2</ls>\3 <ls n="\1">\4</ls>',newline)
 return newline

regex_u1_raw = r'<ls>([^<]*?) ([0-9]+[.]?)</ls>([,;]) ([0-9]+[.]?)'
regex_u1 = re.compile(regex_u1_raw)
def update_1(line):
 newline = line
 while True:
  m = re.search(regex_u1,newline)
  if not m:
   break
  newline = re.sub(regex_u1,r'<ls>\1 \2</ls>\3 <ls n="\1">\4</ls>',newline)
 return newline

regex_u1a_raw = r'<ls n="([^"]*?)">([0-9]+[.]?)</ls>([,;]) ([0-9]+[.]?)'
regex_u1a = re.compile(regex_u1a_raw)
def update_1a(line):
 newline = line
 while True:
  m = re.search(regex_u1a,newline)
  if not m:
   break
  newline = re.sub(regex_u1a,r'<ls n="\1">\2</ls>\3 <ls n="\1">\4</ls>',newline)
 return newline
...
if __name__=="__main__":
 ...
 fileout1 = sys.argv[4] # change transactions
 ...
 for iline,line in enumerate(lines):
  newline = update_2(line)
  newline = update_2a(newline)
  newline = update_1(newline)
  newline = update_1a(newline)
  if newline == line:
   continue
  lines[iline] = newline
  count = count + 1
