...
class Change(object):
 def __init__(self,metaline,page,iline,old,new,reason,iline1,old1,new1):
...
def change1(line):
 reason = 'P.'
 newline = re.sub(r' (P[.] [IV]+[.] [0-9]+[.]) *$',r' <lbinfo n="ls:\1+"/>',line)
 newline = re.sub(r'<ls>Ś.</ls>$','<lsx>Ś.</ls>',line)
...
def init_changes(lines):
 ...
 for iline,line in enumerate(lines):
  ...
  if line.startswith('[Page'):
   page = line
   continue
  reason,newline = change1(line)
  if newline == line:
   continue
  newline = newline.replace('<lsx>','<ls>')
  # next line
  iline1 = iline + 1
  line1 = lines[iline1]
  if line1.startswith('[Page'):
   iline1 = iline1 + 1
   line1 = lines[iline1]
  m1 = re.search('<>([0-9][.]?)',line1)
  if m1:
   k = m1.group(1)
  else:
   continue
  newline = re.sub(r'<ls>(.*?)</ls>$',r'<lbinfo n="ls:\1+%s"/>'%k,newline)
  new1 = re.sub(r'<>([0-9][.]?)',r'<><ls>Ś. %s</ls>'%k,line1)
  change = Change(metaline,page,iline,line,newline,reason,iline1,line1,new1)
...
def change_out(change,ichange):
 ...
 old1 = change.old1
 outarr.append('%s old %s' % (lnum1,old1))
 new1 = change.new1
 outarr.append('%s new %s' % (lnum1,new1))
 outarr.append(';')
 return outarr
