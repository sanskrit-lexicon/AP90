...
def init_changes(lines):
 ...
 for iline,line in enumerate(lines):
  ...
  if iline1 != None:
   ...
   if change.new2 != None:
    line = change.new2
  if re.search(r'[^-]-#} +$',line): # one or more spaces
   iline1 = iline
   line1 = line
  else:
   iline1 = None
...
def compute_new(change):
 """compute new1 and new2
  ...
  old1 = a1 + '{#' + c1 + a2 +'-#}',  
         where a2 is composed of letters only (possibly ending "'" or º_
         
  old2 = '<>{#' + b1 + b2 
          where b1 is composed of letters only (first letter can be 
          apostrophe (representing avagraha) or
          º  [Not sure what this means]
  Construct:
   wordold = a2 + '-' + b1
   wordnew = a2 + b1
   trace = a2 + '#' + b1      The '#' a reminder of Devanagari
   new1 = a1 + ' ' + <lbinfo n="trace"/>
   new2 = '<>{#' + a2 + b1 + b2 

 """
 m1 = re.search(r"^(.*?){#([^#]*?)([a-zA-Zº~]+[']?)-#} +$",change.old1)
 if not m1:
  return
 a1 = m1.group(1)
 c1 = m1.group(2)
 a2 = m1.group(3)
 m2 = re.search(r"^<>{#(['º]?[a-zA-Z]+)(.*)$",change.old2)
 if not m2:
  return
 b1 = m2.group(1)
 b2 = m2.group(2)
 ...
 change.new1 = a1 + '{#' + c1 + '#}' + trace
 change.new1 = change.new1.replace('{##}','')  # when c1 == ''
